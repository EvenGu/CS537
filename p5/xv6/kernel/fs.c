...
#define GETADDR  0x00ffffff;
#define GETCHECK 0xff000000;

#define min(a, b) ((a) < (b) ? (a) : (b))
static void itrunc(struct inode*);

static void writeSum(uchar *data, uint * addrs){
  int i;
  uint sum = 0;
  for(i =0; i< 512; i++){
    sum ^= data[i];
...
static int checkSum(uchar *data, uint addrs){
  int i;
  uint sum = 0;
  for(i =0; i< 512; i++){
    sum ^= data[i];
...
readi(struct inode *ip, char *dst, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    uint address = bmap(ip, off/BSIZE);
    bp = bread(ip->dev, address);
    blockNumber = off/BSIZE;
    ...
    if(ip->type == T_CHECKED){
      if(blockNumber < NDIRECT){
        a = (uint*)bp->data;
        if(checkSum((uchar *)a, ip->addrs[blockNumber]) == -1){
          brelse(bp);
          cprintf("Faileddirect\n");
          return -1;
        }
      }else{
				//if it is an indirect block
        blockNumber -= NDIRECT;
        addr = ip->addrs[NDIRECT];
        temp = bread(ip->dev, addr);
        a = (uint *)temp->data;
        if(checkSum(bp->data, a[blockNumber]) == -1){
			    brelse(temp);
          cprintf("Failedindirect\n");
          return -1;		
				}
        brelse(temp);
      }
...
writei(struct inode *ip, char *src, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    ...
    if(ip->type == T_CHECKED){
      if(blockNumber < NDIRECT){
        writeSum(bp->data, &(ip->addrs[blockNumber]));
      }else{
        //if it is an indirect block
        blockNumber -= NDIRECT;
        addr = ip->addrs[NDIRECT];
        temp = bread(ip->dev, addr);
        a = (uint*)temp->data;
				writeSum(bp->data, &a[blockNumber]);
				bwrite(temp);
        brelse(temp); 
      }
    }
